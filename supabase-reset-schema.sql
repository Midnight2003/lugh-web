...
CONSTRAINT files_source_node_fk FOREIGN KEY (source_node_id) REFERENCES public.nodes (id) ON DELETE SET NULL
);

CREATE INDEX IF NOT EXISTS files_student_id_idx ON public.files (student_id);
CREATE INDEX IF NOT EXISTS files_source_node_id_idx ON public.files (source_node_id);
